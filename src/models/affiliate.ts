...
export async function getAffiliateSummary(user_uuid: string) {
  ...
  const summary = {
    ...
    total_paid: 0,
    total_reward: 0,
    total_quotas: {} as Record<string, number>,
  };
  ...
  data.forEach((item: any) => {
    ...
    if (item.paid_amount > 0 || item.status === "completed") {
      paid_users.add(item.user_uuid);
      summary.total_reward += item.reward_amount;

      // 累计各条记录的实际奖励配额
      if (item.reward_quotas && typeof item.reward_quotas === "object") {
        Object.entries(item.reward_quotas).forEach(([type, count]: [string, any]) => {
          summary.total_quotas[type] = (summary.total_quotas[type] || 0) + Number(count || 0);
        });
      }
    }
  });
