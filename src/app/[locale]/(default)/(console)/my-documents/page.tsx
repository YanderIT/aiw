  ...
  const fetchDocuments = useCallback(async () => {
    try {
      ...
      if (data.success) {
        ...
        // 批量查询润色文档的支付状态
        const polishingDocs = (data.data.documents as Document[]).filter(
          (d: Document) => d.document_type === DocumentType.StudyAbroadConsultation
        );
        if (polishingDocs.length > 0) {
          const statusMap: Record<string, string> = {};
          await Promise.all(
            polishingDocs.map(async (doc: Document) => {
              try {
                const res = await fetch(`/api/orders/check-polishing-status?document_uuid=${doc.uuid}`);
                const result = await res.json();
                if (result.code === 0) {
                  statusMap[doc.uuid] = result.data?.status || "unpaid";
                }
              } catch {
                statusMap[doc.uuid] = "unpaid";
              }
            })
          );
          setPaymentStatusMap((prev: Record<string, string>) => ({ ...prev, ...statusMap }));
        }
      }
    } catch (error) {
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {todayDocuments.map((doc) => (
                  <DocumentCard
                    ...
                    paymentStatus={paymentStatusMap[doc.uuid]}
                  />
                ))}
              </div>
            ) : (
