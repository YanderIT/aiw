...
export default function TargetProgramModule() {
  ...
  const applicationYearOptions = useMemo(() => {
    ...
    for (let y = currentYear; y <= currentYear + 3; y++) {
      // Spring: 只显示当前年份之后的（如果已过3月则跳过当年 Spring）
      if (y > currentYear || currentMonth < 3) {
        options.push({ value: `${y} Spring`, label: `${y} Spring` });
      }
      // Fall: 只显示当前年份之后的（如果已过9月则跳过当年 Fall）
      if (y > currentYear || currentMonth < 9) {
        options.push({ value: `${y} Fall`, label: `${y} Fall` });
      }
    }
          ...
          <div className="grid grid-cols-2 gap-6">
            <div className="space-y-3">
              ...
              >
                ...
                <SelectContent>
                  {applicationYearOptions.map((opt: { value: string; label: string }) => (
